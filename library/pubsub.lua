...
function batteries.pubsub:publish(event, ...)
end
...
function batteries.pubsub:subscribe(event, callback)
end
...
---@param event string The name of an event or `"everything"`
---@param callback function The callback to call when notified
function batteries.pubsub:subscribe_once(event, callback)
end
