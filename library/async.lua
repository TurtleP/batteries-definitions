...
function batteries.async:call(task, arguments, callback, error_callback)
end
...
---@param callback? function (default `false`)
---@param error_callback? function (default `false`)
function batteries.async:add(coroutine, arguments, callback, error_callback)
end

---Add a task to run after a specified delay.
---@param task function Task to execute
---@param delay number Delay in seconds
function batteries.async:add_timeout(task, delay)
end
...
function batteries.async:update_for_time(time, early_out_stalls)
end

---Stalls the current task
function batteries.async.stall()
end

---Force the current task to wait
---@param duration number Time to wait for
function batteries.async.wait(duration)
end
